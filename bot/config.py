...
class Config:
    def __init__(self):
        try:
            ...
            self.C_LINK = config("C_LINK", default="@SvtAv1Enc")
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ffmpeg -i "{}" -map 0:v:0 -map 0:a? -map 0:s? -map 0:t? -vf "scale=-2:480:flags=bicubic:param0=0:param1=1/2" -c:v libsvtav1 -preset 6 -pix_fmt yuv420p10le -crf 32 -svtav1-params "lp=2:tune=2:variance-boost-strength=2:psy-rd=1.0:qm-min=0:keyint=10s:tile-columns=0:film-grain=0:film-grain-denoise=0" -c:a libopus -b:a 80k -ac 2 "{}" -y',
            )
            ...
            self.REPORT_FAILED = config("REPORT_FAILED", default=False, cast=bool)
